...
def encode_answers(answer_list, answers_vocab):
    ...
    for answer_dict in answer_list:
        ...
        for i, token in enumerate(tokens):
            ...
            embed_a[idx] += 1
        embedded_answers.append(embed_a)
...
def get_data_dict(vocab, num_annots):
...
preprocessed_data = get_data_dict(train_vocab, 500)
pickle.dump(preprocessed_data, open('pre_proc_train_data.pickle', 'wb'))

val_vocab = pickle.load(open('val_vocab.pickle', 'rb'))
preprocessed_data = get_data_dict(val_vocab, 100)
pickle.dump(preprocessed_data, open('pre_proc_val_data.pickle', 'wb'))
